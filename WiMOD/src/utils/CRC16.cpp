...
//! @ingroup Utils
//! <!------------------------------------------------------------------------->
//! @brief CRC16 Implementation for WiMOD HCI interface
//! @version 0.2
//! <!------------------------------------------------------------------------->
//!
...
// include the special AVR pgmspace functions only on AVR platforms
...
// store this big array in flash, if possible
#if defined(ARDUINO_ARCH_AVR)
	const PROGMEM UINT16 CRC16_Table[] =
#else
             ...
             UINT16             initVal)
{
	...
	#if defined(ARDUINO_ARCH_AVR)
			tmp =  pgm_read_word_near(CRC16_Table + ((crc ^ *data++) & 0x00FF));
			crc = (crc >> 8) ^ tmp;
	#else
			crc = (crc >> 8) ^ CRC16_Table[(crc ^ *data++) & 0x00FF];
	#endif
